...
/* ============ 8b. регрессия: после отсчёта машина едет САМА, без клавиш ============ */
// Ровно тот баг, на который жаловался игрок: соперники уехали, а машина стоит
// и не реагирует на нитро, потому что разгон требовал удержания газа.
N.start(true);                                        // с отсчётом
N.tick(210, {});                                      // отсчёт кончился, клавиши не нажаты
const idle = N.info();
check('после отсчёта машина трогается без нажатых клавиш', idle.speed > 2000,
      'скорость без клавиш: ' + Math.round(idle.speed));
const distBefore = idle.distance;
N.tick(180, {});                                      // 3 секунды вообще без управления
const roll = N.info();
check('машина продолжает ехать сама', roll.distance > distBefore + 50,
      'проехала ' + Math.round(roll.distance - distBefore) + ' м за 3 с');

// нитро с места: игрок стоит, включает нитро — должен поехать
N.start();
API.player.speed = 0;
API.player.nitro = 100;
N.tick(90, { ShiftLeft: true });
check('нитро поднимает машину с нуля', N.info().speed > 3000,
      'скорость после нитро с места: ' + Math.round(N.info().speed));
